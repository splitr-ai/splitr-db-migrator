...
class GuestUsersPhoneIndexMigrationIntegrationTest {
    ...
    @Test
    @DisplayName("idx_guest_users_phone index exists on guest_users table")
    void testIndexExists() {
        List<String> indexes = jdbcTemplate.queryForList(
                "SELECT indexname FROM pg_indexes " +
                "WHERE tablename = 'guest_users' AND indexname = 'idx_guest_users_phone'",
                String.class
        );
        assertThat(indexes).containsExactly("idx_guest_users_phone");
    }

    @Test
    @DisplayName("idx_guest_users_phone is a partial index with WHERE phone IS NOT NULL")
    ...
    @Test
    @DisplayName("idx_guest_users_phone indexes the phone column")
    void testIndexColumn() {
        List<String> columns = jdbcTemplate.queryForList(
                "SELECT a.attname FROM pg_index i " +
                "JOIN pg_class c ON c.oid = i.indexrelid " +
                "JOIN pg_attribute a ON a.attrelid = i.indrelid AND a.attnum = ANY(i.indkey) " +
                "WHERE c.relname = 'idx_guest_users_phone'",
                String.class
        );
