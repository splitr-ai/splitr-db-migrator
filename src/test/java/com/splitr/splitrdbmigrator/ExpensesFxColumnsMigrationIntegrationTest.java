...
class ExpensesFxColumnsMigrationIntegrationTest {
    ...
    @Test
    @DisplayName("converted_amount column exists on expenses with correct type")
    void testConvertedAmountColumn() {
        Map<String, Object> col = jdbcTemplate.queryForMap(
                "SELECT data_type, is_nullable FROM information_schema.columns " +
                "WHERE table_name = 'expenses' AND column_name = 'converted_amount'"
        );
    ...
    @Test
    @DisplayName("converted_currency column exists on expenses with correct type")
    ...
    @Test
    @DisplayName("fx_snapshot_id column exists on expenses with correct type")
    void testFxSnapshotIdColumn() {
        Map<String, Object> col = jdbcTemplate.queryForMap(
                "SELECT data_type, is_nullable, character_maximum_length FROM information_schema.columns " +
                "WHERE table_name = 'expenses' AND column_name = 'fx_snapshot_id'"
        );
        assertThat(col.get("data_type")).isEqualTo("character varying");
        assertThat(col.get("is_nullable")).isEqualTo("YES");
    ...
    @Test
    @DisplayName("foreign key from expenses.fx_snapshot_id to fx_snapshots.id exists")
    void testForeignKeyExists() {
        Integer count = jdbcTemplate.queryForObject(
                "SELECT COUNT(*) FROM information_schema.table_constraints " +
                "WHERE table_name = 'expenses' AND constraint_type = 'FOREIGN KEY' " +
                "AND constraint_name = 'fk_expenses_fx_snapshot'",
                Integer.class
        );
    ...
    void testConvertedCurrencyCheckConstraint() {
        List<Map<String, Object>> constraints = jdbcTemplate.queryForList(
                ...
                "AND constraint_name = 'chk_expenses_converted_currency'"
        );
    ...
    void testAllFxColumnsNullable() {
        List<Map<String, Object>> columns = jdbcTemplate.queryForList(
                ...
                "WHERE table_name = 'expenses' AND column_name IN ('converted_amount', 'converted_currency', 'fx_snapshot_id') " +
